...
export default function MemberManagement() {
    ...
    const filteredMembers = members.filter((member) => {
        ...
        const matchesLocation =
            selectedLocation === "all" || member.membership.locationId === selectedLocation;
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            setSelectedMembers(checked ? filteredMembers.map((m) => m.membership.id) : []);
        }
    };
    ...
    const fetchMembers = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/studios/${context.currentStudio?.id}/members`, {
                headers: {
                    Authorization: `Bearer ${context.authToken}`
                }
            });
    ...
    const fetchApplications = async () => {
        ...
        try {
            ...
            const mapped: MembershipApplication[] = (body.applications || []).map((row: any) => {
                const profile = row.profiles ?? {};
