...
export async function GET(req: Request, { params }: { params: { studioId: string } }) {
    ...
    try {
        ...
        if (error) {
            console.error("GET /api/admin/studios/[studioId]/kilns fetch error", error);
            return NextResponse.json({ error: "Failed to load kilns" }, { status: 500 });
        }
        ...
        const transformedKilns = (data ?? []).map((kiln: any) => ({
            ...
            warrantyExpiry: kiln.warranty_expiry
                ? new Date(kiln.warranty_expiry).toISOString()
                : undefined,
...
export async function POST(req: Request, { params }: { params: { studioId: string } }) {
    ...
    try {
        ...
        if (error || !data) {
            ...
            return NextResponse.json({ error: "Failed to create kiln" }, { status: 500 });
        }
