...
export async function GET(req: Request, { params }: { params: { studioId: string } }) {
    ...
    try {
      ...
      `
        ...
        if (error) {
            console.error("GET /api/admin/studios/[studioId]/kiln-firings fetch error", error);
            return NextResponse.json({ error: "Failed to load kiln firings" }, { status: 500 });
        }
...
export async function POST(req: Request, { params }: { params: { studioId: string } }) {
    ...
    if (!kilnId) {
        return NextResponse.json({ error: "Missing required field: kilnId" }, { status: 400 });
    }
    ...
    try {
        ...
        if (error || !data) {
            console.error("POST /api/admin/studios/[studioId]/kiln-firings insert error", error);
            return NextResponse.json({ error: "Failed to create kiln firing" }, { status: 500 });
        }
