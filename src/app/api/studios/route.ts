...
export async function POST(req: Request) {
    ...
    if (
        ...
        return NextResponse.json(
            {
                error: "Missing required fields (studioName, studioHandle, city, state, locationName, locationAddress, locationZip, locationPhone)"
            },
            { status: 400 }
        );
    }
    ...
    try {
        ...
        if (locationError || !location) {
            console.error("POST /api/studios insert location error", locationError);
            return NextResponse.json(
        ...
        if (membershipError || !membership) {
            console.error("POST /api/studios insert membership error", membershipError);
            return NextResponse.json(
