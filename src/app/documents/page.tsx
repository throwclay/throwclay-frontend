...
export function StudioDocuments() {
  ...
  const handleUnpublishDocument = (docId: string) => {
    setDocuments(
      documents.map((doc) =>
        doc.id === docId ? { ...doc, status: "draft" as const } : doc
      )
    );
  };
  ...
  const removeContentBlock = (blockId: string) => {
    if (editingDocument) {
      setEditingDocument({
        ...editingDocument,
        content: editingDocument.content.filter(
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage policies, guidelines, and instructional materials for your
            studio
          </p>
        </div>
                ...
                <div>
                  ...
                  <ScrollArea className="h-[200px] border rounded-md p-3">
                    <div className="space-y-3">
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  const current = newDocument.audience || [];
                                  setNewDocument({
                                    ...newDocument,
                                    audience: checked
                                      ? [...current, type.id]
                                      : current.filter((a) => a !== type.id),
                                  });
                                }}
                              />
                      ...
                      <div>
                        <p className="text-sm font-medium mb-2">Staff Roles</p>
                        <div className="space-y-2">
                          {staffRoles.map((role) => (
                            <div
                              key={role.id}
                  ...
                  <Tabs defaultValue="members" className="w-full">
                    ...
                    <TabsContent value="members" className="space-y-2 mt-3">
                      {membershipTypes.map((type) => (
                        ...
                        >
                          <Checkbox
                            checked={editingDocument.audience.includes(type.id)}
                            onCheckedChange={() => toggleAudience(type.id)}
                          />
                          <label className="text-sm">{type.label}</label>
                        </div>
                      ))}
                    </TabsContent>
                    <TabsContent value="staff" className="space-y-2 mt-3">
                      {staffRoles.map((role) => (
                        <div
                          key={role.id}
                          className="flex items-center space-x-2"
                        >
                          <Checkbox
                            checked={editingDocument.audience.includes(role.id)}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                      ...
                      <Button
                        ...
                        className="w-full"
                        onClick={() =>
                          window.open(editingDocument.googleDocsUrl, "_blank")
                        }
                      >
