...
export function calculateShelfCapacity(
    ...
    const actualUtilization = shelfArea > 0 ? Math.min(totalUsedArea / shelfArea, 1) : 0;
    ...
    const estimatedWeight = pieceSize.weight ? piecesPerShelf * pieceSize.weight : undefined;
...
export function calculateTotalCapacity(
    ...
    if (typeof pieceSize === "string") {
        if (pieceSize === "custom" && customPieceSize) {
            ...
            actualPieceSize =
                PIECE_SIZE_PRESETS[pieceSize]?.dimensions || PIECE_SIZE_PRESETS.medium.dimensions;
        }
    } else {
...
export function calculateCapacityByFiringType(
    ...
    const utilizationByType: Record<string, number> = {
        bisque: 0.9, // Can pack tighter
        glaze: 0.75, // Need more space
        raku: 0.6, // Safety spacing
        crystalline: 0.8 // Precise but can be efficient
    };
...
export function calculateNextMaintenance(
    ...
    if (
        intervalType === "firings" &&
        currentFiringCount !== undefined &&
        lastFiringCount !== undefined
    ) {
        ...
        nextMaintenanceDate = new Date(
            lastMaintenanceDate.getTime() + interval * 24 * 60 * 60 * 1000
        );
    }
...
export function validateShelfConfiguration(
    ...
    if (totalShelfHeight > totalKilnHeight) {
        errors.push(
            `Total shelf height (${totalShelfHeight}") exceeds kiln height (${totalKilnHeight}")`
        );
    }
    ...
    shelves.forEach((shelf, index) => {
        if (shelf.height < 6) {
            warnings.push(
                `Shelf ${shelf.level} has very low clearance (${shelf.height}"). Minimum recommended is 8".`
            );
        }
        if (shelf.height > 16) {
            warnings.push(
                `Shelf ${shelf.level} has very high clearance (${shelf.height}"). This may reduce capacity.`
            );
        }
    });
