...
export function TimeCardManagement() {
    ...
    const currentPayPeriod =
        context.currentStudio?.payPeriods?.find((pp) => pp.status === "open") ||
        context.currentStudio?.payPeriods?.[0];

    const filteredTimeCards = timeCards.filter((timeCard) => {
        const matchesSearch = timeCard.employeeName
            .toLowerCase()
            .includes(searchTerm.toLowerCase());
        const matchesStatus = statusFilter === "all" || timeCard.status === statusFilter;
        const matchesPayPeriod =
            selectedPayPeriod === "current" || timeCard.payPeriodId === selectedPayPeriod;
    ...
    const getSummaryStats = () => {
        const pending = timeCards.filter(
            (tc) => tc.status === "submitted" || tc.status === "needs-review"
        ).length;
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <TableBody>
                            {filteredTimeCards.map((timeCard) => {
                                const employee = employees.find(
                                    (emp) => emp.id === timeCard.employeeId
                                );
                                const payPeriod = context.currentStudio?.payPeriods?.find(
                                    (pp) => pp.id === timeCard.payPeriodId
                                );
                                const pay = employee
                                    ? calculateHourlyPay(
                                          employee,
                                    ...
                                    <TableRow key={timeCard.id}>
                                        ...
                                        <TableCell>
                                            <span className="font-medium">
                                                {timeCard.totalHours.toFixed(1)}h
                                            </span>
                                        </TableCell>
                                        <TableCell>
                                            {timeCard.totalRegularHours.toFixed(1)}h
                                        </TableCell>
                                        <TableCell>
            ...
            <Dialog
                open={!!selectedTimeCard && !showApprovalDialog}
                onOpenChange={() => setSelectedTimeCard(null)}
            >
                <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>
                            Time Card Details - {selectedTimeCard?.employeeName}
                        </DialogTitle>
                        <DialogDescription>
                                ...
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <p className="text-xl font-bold">
                                            {selectedTimeCard.totalHours.toFixed(1)}
                                        </p>
                                    </CardContent>
                                </Card>
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <p className="text-xl font-bold">
                                            {selectedTimeCard.totalRegularHours.toFixed(1)}
                                        </p>
                                    </CardContent>
                                </Card>
                                <Card>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <Button
                                ...
                                onClick={() =>
                                    selectedTimeCard &&
                                    handleApproveTimeCard(selectedTimeCard.id, "rejected")
                                }
                            >
                            ...
                            <Button
                                onClick={() =>
                                    selectedTimeCard &&
                                    handleApproveTimeCard(selectedTimeCard.id, "approved")
                                }
                            >
