...
interface ClassSettingsBadgesProps {
  ...
  attendanceRecords: Record<string, { percentage: number; sessionsAttended: number; totalSessions: number }>;
...
export function ClassSettingsBadges({
  ...
  const saveBadgeDesign = (design: BadgeDesign) => {
    const newBadge: ClassBadge = {
      ...
      classId,
      name: design.name,
      description: selectedBadge?.description || 'Complete the class requirements to earn this badge',
      design,
  ...
  const getBadgeStats = (badgeId: string) => {
    const awarded = studentBadges.filter(sb => sb.badgeId === badgeId && sb.status === 'active').length;
  ...
  const renderBadgePreview = (badge: ClassBadge, size: 'small' | 'medium' = 'medium') => {
    ...
    const gradientStyle = design.gradientEnabled && design.gradientColors ? {
      background: `linear-gradient(${
        ...
        design.gradientDirection === 'horizontal' ? '90deg' : '45deg'
    ...
    <div className="space-y-6">
      <Tabs defaultValue="badges" className="space-y-6">
        ...
        <TabsContent value="badges" className="space-y-6">
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="text-2xl font-semibold">{classBadges.length}</div>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="text-2xl font-semibold">
                      {studentBadges.filter(sb => sb.status === 'active').length}
                    </div>
              ...
              {classBadges.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Create your first badge to motivate and reward student achievements
                  </p>
                  <Button onClick={createNewBadge}>
                    <Plus className="w-4 h-4 mr-2" />
                    Create First Badge
                      ...
                      <div key={badge.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-2">
                              ...
                              <Badge variant={badge.isEnabled ? 'default' : 'secondary'}>
                                {badge.isEnabled ? 'Active' : 'Inactive'}
                              </Badge>
                              {badge.autoAward && (
                                <Badge variant="outline">Auto Award</Badge>
                              )}
                            </div>
                            ...
                            <div className="flex items-center space-x-4 mt-2 text-xs text-muted-foreground">
                              <span>{stats.awarded} / {stats.total} awarded</span>
                              <span>{stats.percentage.toFixed(0)}% completion</span>
                              <span>{badge.requirements.length} requirements</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={() => toggleBadgeEnabled(badge.id)}
                          />
                          <Button
                            variant="ghost"
                            size="sm"
          ...
          <Dialog open={showBadgeDesigner} onOpenChange={setShowBadgeDesigner}>
              ...
              {selectedBadge && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const updatedBadge = { ...selectedBadge, name: e.target.value };
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={(checked) => {
                              const updatedBadge = { ...selectedBadge, isEnabled: checked };
                              setSelectedBadge(updatedBadge);
                            }}
                          />
                          <Label htmlFor="badge-enabled">
                            {selectedBadge.isEnabled ? 'Active' : 'Inactive'}
                          </Label>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => {
                          const updatedBadge = { ...selectedBadge, description: e.target.value };
                          setSelectedBadge(updatedBadge);
                        }}
                        rows={3}
                      />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) => {
                            const updatedBadge = { ...selectedBadge, autoAward: checked };
                            setSelectedBadge(updatedBadge);
                          }}
                        />
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) => {
                            const updatedBadge = { ...selectedBadge, manualReview: checked };
                            setSelectedBadge(updatedBadge);
                          }}
                        />
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          const updatedBadge = { ...selectedBadge, revokable: checked };
                          setSelectedBadge(updatedBadge);
                        }}
                      />
                      ...
                      {selectedBadge.requirements.map((req, index) => (
                        <div key={req.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div className="flex-1">
                            ...
                            <div className="mt-1 text-sm">{req.description}</div>
                          </div>
                  ...
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button onClick={() => {
                      updateBadgeSettings(selectedBadge.id, selectedBadge);
        ...
        <TabsContent value="awarding" className="space-y-6">
