...
export function BadgeAwardingService({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'awarded': return <Check className="w-4 h-4" />;
      case 'eligible': return <Trophy className="w-4 h-4" />;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Select value={autoAwardMode} onValueChange={(value: any) => setAutoAwardMode(value)}>
          ...
          <Button onClick={performAutoAward} disabled={stats.totalEligible === 0}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Eligible for Award</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="space-y-4">
            ...
            {selectedBadgeData && selectedBadge !== 'all' && (
              <div className="flex items-center space-x-4 p-4 bg-muted rounded-lg">
                ...
                <div className="flex items-center space-x-2">
                  <Badge variant={selectedBadgeData.autoAward ? 'default' : 'secondary'}>
                    {selectedBadgeData.autoAward ? 'Auto Award' : 'Manual Review'}
                  </Badge>
                  <Badge variant={selectedBadgeData.isEnabled ? 'default' : 'outline'}>
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <Avatar>
                      <AvatarFallback>
                        {eligibility.studentName.split(' ').map(n => n[0]).join('')}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-medium">{eligibility.studentName}</div>
                      <div className="text-sm text-muted-foreground">
                        {badge?.name} - {eligibility.overallProgress.toFixed(0)}% complete
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <Avatar>
                    <AvatarFallback>
                      {selectedEligibility.studentName.split(' ').map(n => n[0]).join('')}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="font-medium">{selectedEligibility.studentName}</div>
                    <div className="text-sm text-muted-foreground">
                      {classBadges.find(b => b.id === selectedEligibility.badgeId)?.name}
                    </div>
                  </div>
                </div>
                <div className="text-right">
              ...
              <div className="space-y-4">
                ...
                {selectedEligibility.requirements.map((req) => (
                  <div key={req.requirementId} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        req.isMet ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                      }`}>
                        {req.isMet ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}
                      </div>
                      <div>
              ...
              <div className="flex justify-end space-x-2">
                ...
                {selectedEligibility.isEligible && !selectedEligibility.hasExistingBadge && (
                  <Button onClick={() => {
                    handleManualAward(
