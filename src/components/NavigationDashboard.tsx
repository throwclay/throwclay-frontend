...
export function NavigationDashboard({ onLogout }: NavigationProps) {
    const context = useAppContext();
    const pathname = usePathname();
    const router = useRouter();

    const _pendingInvites = context.pendingInvites.filter((i) => i.status === "pending").length;
    ...
    const hasAnyStudioMemberships = (user: any) => Boolean((user as any)?.hasStudioMemberships);

    // Main navigation items based on user type
    type NavItem = { id: string; label: string; icon: any; description?: string };

    console.log(`User is active membership: ${hasAnyStudioMemberships(context.currentUser)}`);
                                    ...
                                    <div className="w-full space-y-2">
                                        ...
                                        {notification.type === "invite" && (
                                            <div className="flex gap-2 mt-2">
                                                <Link href="invites">
                                                    <Button size="sm">Review invite</Button>
                                                </Link>
                                        ...
                                        <div className="flex items-center space-x-3 p-4">
                                            ...
                                            <div className="flex flex-col space-y-1 flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    ...
                                                    location
                                                    {(context.currentStudio.locations?.length ||
                                                        0) !== 1
                                                        ? "s"
                                                        : ""}
                                                </p>
