...
export function BadgeManagement({
  ...
  const mockClassBadges: ClassBadge[] = [
    {
      ...
      description: 'Mastered the art of wheel throwing with consistent centering and pulling techniques',
      design: {
        id: 'design_1',
        name: 'Wheel Master',
        shape: 'circle',
  ...
  const filteredBadges = badges.filter(badge => {
    const matchesSearch = badge.studentName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const renderBadgePreview = (badge: StudentBadge, size: 'small' | 'medium' | 'large' = 'medium') => {
  ...
  const getBadgeStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'revoked': return <X className="w-4 h-4 text-red-500" />;
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-2xl font-semibold">{badges.filter(b => b.status === 'active').length}</div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-2xl font-semibold">
                  {Math.round(badges.filter(b => b.status === 'active').reduce((sum, b) => sum + (b.finalAttendance || 0), 0) / Math.max(badges.length, 1))}%
                </div>
      ...
      <div className="flex items-center space-x-4">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder={isStudioView ? "Search by student name..." : "Search badges..."}
            value={searchTerm}
                ...
                <div className="flex items-start justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => handleViewBadgeDetails(badge)}>
                        <Eye className="w-4 h-4 mr-2" />
                        View Details
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleShareBadge(badge)}>
                        <Share2 className="w-4 h-4 mr-2" />
                        Share Badge
                      </DropdownMenuItem>
                      {badge.certificateUrl && (
                        <DropdownMenuItem onClick={() => handleDownloadCertificate(badge)}>
                ...
                <div className="space-y-2">
                  ...
                  {badge.finalGrade && (
                    <>
                      ...
                      <Progress value={badge.finalGrade} className="h-1" />
                    </>
                  )}
                </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-semibold">
                    {mockClassBadges.find(cb => cb.id === selectedBadge.badgeId)?.name}
                  </h3>
                  <p className="text-muted-foreground">
                    {mockClassBadges.find(cb => cb.id === selectedBadge.badgeId)?.description}
                  </p>
                  <div className="flex items-center space-x-2 mt-2">
                    {getBadgeStatusIcon(selectedBadge.status)}
                    <Badge variant={selectedBadge.status === 'active' ? 'default' : 'destructive'}>
                      {selectedBadge.status === 'active' ? 'Active' : 'Revoked'}
                    </Badge>
                    <Badge variant="secondary">
                      {selectedBadge.awardMethod === 'automatic' ? 'Auto Awarded' : 'Manual Award'}
                    </Badge>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Awarded Date:</span>
                      <span>{new Date(selectedBadge.awardedDate).toLocaleDateString()}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-xs">{selectedBadge.verificationCode}</span>
                    </div>
                    {isStudioView && (
                      <div className="flex justify-between">
