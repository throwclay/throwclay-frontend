...
export function MemberIntakeFormBuilder() {
  ...
  const fieldTypes = [
    ...
    { type: 'radio', label: 'Radio Buttons', icon: Radio, description: 'Single selection from options' }
  ];
      ...
      case 'radio':
        return (
          <RadioGroup>
            {field.options?.map((option, i) => (
              <div key={i} className="flex items-center space-x-2">
                <RadioGroupItem value={option.toLowerCase().replace(/\s+/g, '-')} id={`${field.id}-${i}`} />
                <Label htmlFor={`${field.id}-${i}`}>{option}</Label>
              </div>
            ))}
          </RadioGroup>
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Customize your membership application form</p>
        </div>
                ...
                <div className="space-y-6">
                  {formFields.length === 0 ? (
                    <div className="text-center py-16 space-y-4">
                      ...
                      <div className="space-y-2">
                        <h3 className="text-xl font-semibold">No fields added yet</h3>
                        <p className="text-muted-foreground">Click on a field type to add it to your form</p>
                      </div>
                    </div>
                  ) : (
                    formFields.map((field, index) => (
                      <div key={field.id} className="border rounded-lg p-6 space-y-4 hover:border-primary/50 transition-colors">
                        <div className="flex items-center justify-between">
